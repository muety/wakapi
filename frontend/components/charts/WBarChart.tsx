...
function WBarChartComponent({
  ...
  const handleMouseOver = (
    ...
    if (orientation === "vertical") {
      ...
      const barX = xScale(data.key as any);
      const barY = yScale(data.total as any);

      tooltipX = (barX ?? 0) + barWidth / 2 + margin.left;
      tooltipY = (barY ?? 0) + margin.top - 10;
    } else {
      const barHeight = (yScale as ScaleBand<string>).bandwidth();
      const barX = xScale(data.total as any);
      const barY = yScale(data.key as any);

      tooltipX = (barX ?? 0) + margin.left + 10;
      tooltipY = (barY ?? 0) + barHeight / 2 + margin.top;
    }
