...
func TestAuthenticateMiddleware_tryHandleOidc_ValidToken_WithCustomClaims(t *testing.T) {
	config.Set(config.Empty())
...
func TestAuthenticateMiddleware_tryHandleOidc_ExpiredToken_WithCustomClaims_ExistingUser(t *testing.T) {
	const (
		testProvider = "mock"
		testSub      = "testsub"
	)
	...
	testIdToken := &config.IdTokenPayload{
		Subject:           testSub,
		PreferredUsername: "preferred_user",
		Expiry:            time.Now().Add(-time.Minute).Unix(), // expired
		ProviderName:      testProvider,
		UsernameClaim:     "custom_claim",
		AllClaims: map[string]interface{}{
			"custom_claim": "custom_value",
