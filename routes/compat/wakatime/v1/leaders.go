...
func (h *LeadersHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: language filter param
	pageParams := utils.ParsePageParamsWithDefault(r, 1, 100)
	by := models.SummaryLanguage

	globalLeaderboard, err := h.leaderboardSrvc.GetByInterval(h.leaderboardSrvc.GetDefaultScope(), pageParams, true)
	if err != nil {
	...
	if user != nil && !globalLeaderboard.HasUser(user.ID) {
		if l, err := h.leaderboardSrvc.GetByIntervalAndUser(h.leaderboardSrvc.GetDefaultScope(), user.ID, true); err == nil {
			globalLeaderboard.AddMany(l)
		} else {
	...
	vm := h.buildViewModel(globalLeaderboard, languageLeaderboard, user, h.leaderboardSrvc.GetDefaultScope(), pageParams)
