...
func TestBadgeHandler_Get(t *testing.T) {
	...
	mockServices.UsersFunc = func() services.IUserService {
		userServiceMock := new(mocks.UserServiceMock)
		userServiceMock.On("GetUserById", "user1").Return(&user1, nil)
		return userServiceMock
	}

	mockServices.SummaryFunc = func() services.ISummaryService {
	...
	apiRouter.Use(middlewares.NewAuthenticateMiddleware(mockServices.Users()).WithOptionalFor("/api/badge/").Handler)
	apiRouter.Get("/badge/{user}/*", api.GetBadge)

	// we might need to create a new apiv1 route and then register the route manually
	...
	t.Run("when requesting badge", func(t *testing.T) {
		t.Run("should return badge", func(t *testing.T) {
			...
			req = utilities.WithUrlParam(req, "user", "user1")
