...
func (a *APIv1) GetMetrics(w http.ResponseWriter, r *http.Request) {
	...
	if userMetrics, err := a.getUserMetrics(reqUser); err != nil {
	...
	if reqUser.IsAdmin {
		if adminMetrics, err := a.getAdminMetrics(reqUser); err != nil {
			conf.Log().Request(r).Error("error occurred", "error", err)
			w.WriteHeader(http.StatusInternalServerError)
			w.Write([]byte(conf.ErrInternalServerError))
			return
		} else {
			metrics = append(metrics, *adminMetrics...)
		}
	}
...
func (a *APIv1) getUserMetrics(user *models.User) (*mm.Metrics, error) {
	var metrics mm.Metrics

	allTimeRequest := summarytypes.NewSummaryRequest(time.Time{}, time.Now(), user)
	options := summarytypes.DefaultProcessingOptions()
	summaryAllTime, err := a.services.Summary().Generate(allTimeRequest, options)
	if err != nil {
	...
	todayRequest := summarytypes.NewSummaryRequest(from, to, user)
	summaryToday, err := a.services.Summary().Generate(todayRequest, options)
	if err != nil {
		conf.Log().Error("failed to retrieve today's summary for metric", "userID", user.ID, "error", err)
		return nil, err
	}

	heartbeatCount, err := a.services.Heartbeat().CountByUser(user)
	if err != nil {
	...
	if a.config.App.LeaderboardEnabled {
		leaderboard, err = a.services.LeaderBoard().GetByIntervalAndUser(a.services.LeaderBoard().GetDefaultScope(), user.ID, false)
		if err != nil {
	...
	// Database metrics
	metricsRepo := &repositories.MetricsRepository{Config: a.config, DB: a.db}
	dbSize, err := metricsRepo.GetDatabaseSize()
	if err != nil {
...
func (a *APIv1) getAdminMetrics(user *models.User) (*mm.Metrics, error) {
	var metrics mm.Metrics
	...
	if t, err := a.services.KeyValue().GetString(conf.KeyLatestTotalTime); err == nil && t != nil && t.Value != "" {
		if d, err := time.ParseDuration(t.Value); err == nil {
			totalSeconds = int(d.Seconds())
		}
	}

	totalUsers, _ := a.services.Users().Count()
	totalHeartbeats, _ := a.services.Heartbeat().Count(true)
	slog.Debug("finished counting users and heartbeats", "duration", time.Since(t0))

	activeUsers, err := a.services.Users().GetActive(false)
	if err != nil {
	...
	userCounts, err := a.services.Heartbeat().CountByUsers(activeUsers)
	if err != nil {
	...
	for i := range activeUsers {
		wp.Submit(func() {
			request := summarytypes.NewSummaryRequest(from, to, activeUsers[i])
			options := summarytypes.DefaultProcessingOptions()
			summary, err := a.services.Summary().Generate(request, options) // using Generate which has caching
			if err != nil {
